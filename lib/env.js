...
async function tryPs(script, timeoutMs = DEFAULT_TIMEOUT_MS) {
    try {
 ...
 */
export async function runEnvDoctor(timeoutMs = DEFAULT_TIMEOUT_MS) {
    const findings = [];
    const suggestions = [];
...
export function renderEnvReport(report) {
    const lines = [];
